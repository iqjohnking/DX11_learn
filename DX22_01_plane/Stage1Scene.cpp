...
bool Stage1Scene::GetIsWin() const
{
	return m_IsWin;
}
...
{
	Game::GetInstance()->GetSound()->Stop(SOUND_LABEL_BGM000);
  ...
  // Win条件：頭以外が全部 Removed
	if (m_EndState == EndState::None)
	{
		if (daruma->GetState() == 2 || daruma->IsHeadRemoved())
		{
			...
			Game::GetInstance()->GetSound()->Play(SOUND_LABEL_SE003);
		}
	...
	case 2: // Power select (0..10 accel loop) -> SPACE apply
		...
		if (Input::GetKeyTrigger(VK_SPACE))
		{
			...
			if (m_HasPendingHit)
			{
				...
				Game::GetInstance()->GetSound()->Play(SOUND_LABEL_SE001);
				m_HasPendingHit = false;
			}
		}
